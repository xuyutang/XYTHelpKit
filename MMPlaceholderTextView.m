...
@interface MMPlaceholderTextView ()
...
-(void)setView{
    if (!self.placeholderLabel) {
        self.placeholderLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
    ...
    if (!self.wordNumLabel) {
        ...
        self.wordNumLabel.textColor = [UIColor colorWithHexString:@"#F1C87B"];
...
-(void)placeholderTextViewdidChange:(NSNotification *)notificat{
    ...
    MMPlaceholderTextView *textView = (MMPlaceholderTextView *)notificat.object;
...
- (void)didChangeText:(void (^)(MMPlaceholderTextView *))block{
